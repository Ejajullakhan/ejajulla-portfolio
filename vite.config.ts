    ...
    configureServer(server) {
      server.middlewares.use("/api/contact", (req, res, next) => {
        ...
        req.on("data", (chunk) => {
          ...
          if (rawBody.length > 20_000) req.destroy();
        });
        req.on("end", async () => {
          ...
          try {
            ...
            res.end(JSON.stringify({ ok: false, error: "Please send valid form data." }));
            return;
          }
          const result = await processContactRequest(req.method, body, req.headers);
